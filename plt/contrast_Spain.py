...
# Read data
data = pd.read_csv('../Data/Compartment_ES.csv').values
N = 46937060
beta_a = 0.1
beta_n = 0.3494
sigma = 0.2
...
multipliers = [0.2220/0.3725, 1.8, 2]
...
def SEIRp1(y, t):
    ...
    dRdt = gamma * y[2]
    dpdt = 0.3725 * y[4] * (1 - y[4]) * (1-(y[2] / N * 510.7))
...
for i, mult in enumerate(multipliers):
    def SEIRp2(y, t):
        ...
        dIdt = sigma * y[1] - gamma * y[2]
        dRdt = gamma * y[2]
        dpdt = mult * 0.3725 * y[4] * (1 - y[4]) * (1-(y[2] / N * 510.7))
        return [dSdt, dEdt, dIdt, dRdt, dpdt]
...
percent_new = -reduction/peak_val_l2
percent_cum= cum_diff/cum_l2


annotation_text = (f'Peak {-days_earlier} days later\n'
                  f'{-reduction:.0f} more peak cases\n'
                  f'{cum_diff:.0f} more cumulative\n'
                   f'cases')

plt.text(peak_date_l2-timedelta(days=27), peak_val_l2*1.05,  # Centered vertically
         annotation_text,
         ha='center', va='top',
