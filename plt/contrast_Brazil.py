...
# Read data
data = pd.read_csv('../Data/Compartment_BR.csv').values
N = 212559409
beta_a = 0.1
beta_n = 0.4944
sigma = 0.2
...
def SEIRp1(y, t):
    ...
    dRdt = gamma * y[2]
    dpdt = 0.1567 * y[4] * (1 - y[4]) * (1-792.3*((y[2] / N + 3.663e-6) * y[0] / N))
...
for i, mult in enumerate(multipliers):
    def SEIRp2(y, t):
        ...
        dRdt = gamma * y[2]
        # dpdt = 0.1567 * y[4] * (1 - y[4]) * (1 - 792.3 * ((y[2] / N + 3.663e-6) * y[0] / N))
        dpdt = 0.1567 * y[4] * (1 - y[4]) * (1-792.3*((y[2] / N + 3.663e-6)))
        return [dSdt, dEdt, dIdt, dRdt, dpdt]
...
plt.text(peak_date_l3+timedelta(2), peak_val_l3*0.5,  # Centered vertically
         annotation_text,
         ha='center', va='top',
...
plt.tight_layout()
plt.savefig("../Fig/Contrast_Brazil.jpg",
           dpi=600)
plt.show()